...
export class AdminAddCategoryPage implements OnInit {
 ...
 async loadCloudFiles(id) {
  ...
  uploadImage() {
    this.category.id = Date.now().toString();
  ...
  async addCategory() {
    ...
    if (this.formValidation()) {
      ...
      try {
        await this.firestore.doc('categorys/' + this.category.id).set({
  ...
  formValidation() {
    ...
    if (!this.category.description) {
      this.showToast('Introduzca la descripción de la categoría');
      return false;
    }

    if (!this.category.img) {
      this.showToast('Seleccione la imagen de la categoría');
      return false;
    }
